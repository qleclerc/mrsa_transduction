...
trace_model = all_params[["hill_burst"]]
...
traj4 = phage_tr_model(params, init.state, seq(0,24,1), "hill", T, F, T)
...
traj5 = phage_tr_model(params, init.state, seq(0,24,1), "hill", T, F, T)
...
traj5$B_tot = traj5$Be+traj5$Bt+traj5$Bet
...
traj3 = phage_tr_model(params, init.state, seq(0,24,1), "hill", T, F, T)
...
pa = ggplot(traj_all) +
  geom_line(aes(time, 50*(1-(Be+Bt+Bet)/2.76e9)+1), size = 1) +
...
pb = ggplot(traj_all) +
  ...
  scale_y_continuous(breaks = c(-2,0,2,4,6,8,10), limits = c(-2,10)) +
