...
choose_model = function(model = "mass"){
  ...
  if(model == "dde_mass_link_L"){
    phagebac_simulateDeterministic <- function(theta,init.state,times) {
      phagebac_dde <- function(time, state, parameters) {
        ...
        dP =  - beta * P * N + N_past * P_past * beta * L*(1-N_past/Nmax)
  ...
  if(model == "dde_frequentist_link_L"){
    phagebac_simulateDeterministic <- function(theta,init.state,times) {
      phagebac_dde <- function(time, state, parameters) {
        ...
        P_inf_past = ifelse(time < tau, 0, (1-exp(-beta *N_past))*P_past)
        B_inf_past = ifelse(time < tau, 0, (1-exp(-P_inf_past/N_past))*N_past)
  ...
  if(model == "tr_mass_link_L"){
    phagebac_simulateDeterministic <- function(theta,init.state,times) {
      phagebac_ode <- function(time, state, parameters) {
        ...
        dBt = Gt*(1 - N/Nmax)*Bt - beta * Bt * P
        dBet = Get*(1 - N/Nmax)*Bet - beta * Bet * P +
          beta * Be * P * L*(1-N/Nmax) * tr * beta * Bt +
          beta * Bt * P * L*(1-N/Nmax) * tr * beta * Be +
          beta * Bet * P * L*(1-N/Nmax) * tr * beta * (Be+Bt)
        
        dP = beta * N * P * L*(1-N/Nmax)*(1-tr)
  ...
  if(model == "tr_frequentist_link_L"){
    phagebac_simulateDeterministic <- function(theta,init.state,times) {
      phagebac_ode <- function(time, state, parameters) {
        ...
        dBt = Gt*(1 - N/Nmax)*Bt - B_inf* (Bt/N)
        dBet = Get*(1 - N/Nmax)*Bet - B_inf* (Bet/N) +
          (1-exp(-B_inf * (Be/N) * L * tr / N))*Bt +
          (1-exp(-B_inf * (Bt/N) * L * tr / N))*Be +
          (1-exp(-B_inf * (Bet/N) * L * tr / N))*(Bt+Be)
        
        dP = B_inf * L * (1-tr) - P_inf
  ...
  if(model == "tr_dde_frequentist_decay_link_both"){
    phagebac_simulateDeterministic <- function(theta,init.state,times) {
      phagebac_dde <- function(time, state, parameters) {
        ...
        dBt = Gt*(1 - N/Nmax)*Bt - B_inf* (Bt/N)
        dBet = Get*(1 - N/Nmax)*Bet - B_inf* (Bet/N) +
          (1-exp(-B_inf_past * (Be/N) * L * tr / N))*Bt +
          (1-exp(-B_inf_past * (Bt/N) * L * tr / N))*Be 
...
target_function_trans = function(theta, model = "mass"){
  
  my_init.state <- c(Be = lab_data_trans5$Be[1], Bt = lab_data_trans5$Bt[1], Bet = 0,
                     P = lab_data_trans5$P[1])
  
  return(dLogPosterior(fitmodel = phagebac_model, theta = theta, init.state = my_init.state, 
                       data = lab_data_trans5, margLogLike = dTrajObs, log = TRUE))
...
#                   init.theta = c(Ge = 1, Gt = 1, Get = 1, Nmax = 1e9),
#                   n.iterations = 100000,
...
# theta = as.vector(tail(mcmc_fit$trace, 1))
# names(theta) = colnames(mcmc_fit$trace)
# 
# traj = mass_model$simulate(theta, init.state, times = c(0:24))
# 
# ggplot() +
#   geom_line(data = traj, aes(time, Be, colour = "Model")) +
#   geom_line(data = traj, aes(time, Bt, colour = "Model")) +
#   geom_line(data = traj, aes(time, Bet, colour = "Model")) +
#   geom_line(data = lab_data, aes(time, Be, colour = "Data")) +
#   geom_line(data = lab_data, aes(time, Bt, colour = "Data")) +
#   geom_line(data = lab_data, aes(time, Bet, colour = "Data")) +
...
all_model_names = c("mass", "mass_decay", "dde_mass_decay",
                    "mass_link_beta", "mass_link_L", "mass_link_both", "dde_mass_link_both",
                    "frequentist", "frequentist_link_both", "dde_frequentist_link_both", 
                    "dde_frequentist_decay_link_both")
all_theta = vector("list", length(all_model_names))
i = 1

for(model_name in all_model_names){
  
  model_name = "mass_link_L"
  phagebac_model = choose_model(model = model_name)
  
  mcmc_fit = mcmcMH(target = target_function_trans,
                    init.theta = c(beta = 8e9, L = 50, phi = 30000, tr = 1e6),
                    n.iterations = 5000,
                    adapt.size.start = 500,
                    adapt.size.cooling = 0.999,
                    adapt.shape.start = 10000)
  ...
  # mcmc.trace = burnAndThin(mcmc.trace, burn = 2000, thin = 10)
  ...
  #theta = c(beta = 8e9, L = 20, phi = 30000)
  #theta["tr"] = 5e6
  
  traj = phagebac_model$simulate(theta, init.state, times = seq(0, 24, 1))
  ...
  filename = paste0("best_fit_", model_name, ".png")
  ggsave(filename)
  
  all_theta[[i]] = mcmc_fit$trace
  names(all_theta)[i] = model_name
  
  i = i + 1
}

saveRDS(all_theta, here::here("Fitting", "fitted_params.rds"))
