...
source(here::here("Models", "transduction_model_functions.R"))

# mass_model = readRDS(here::here("Fitting", "mass_model.rds"))
model = readRDS(here::here("Fitting", "transduction_model.rds"))
...
models_to_try = data.frame(model_name="mass", frequentist=FALSE, delay=TRUE, 
                           fixed_delay=0.3, decay=TRUE,
                           link_beta=FALSE, link_L=FALSE, link_delay=FALSE, transduction=TRUE)
models_to_try = rbind(models_to_try,
                      data.frame(model_name="mass", frequentist=FALSE, delay=TRUE, 
                                 fixed_delay=0.3, decay=TRUE,
                                 link_beta=FALSE, link_L=FALSE, link_delay=FALSE, transduction=TRUE))

all_theta = data.frame()
...
for(i in 1:nrow(models_to_try)){
  ...
  mcmc_fit = run_mcmc(model, lab_data_trans,
                      init.theta = c(beta = 8e9, L = 20, gamma = 30000, alpha = 1e6, tau = 0.8),
                      n.iterations = 10000,
  ...
  all_theta = cbind(all_theta, c(models_to_try$model_name[i], theta))
  
}

saveRDS(all_theta, here::here("best fits", "best_params_transduction.rds"))
