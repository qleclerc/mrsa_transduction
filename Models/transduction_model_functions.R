  ...
  if(delay){
    ...
    model_simulateDeterministic <- function(theta, init.state, times) {
      ...
      model_dde <- function(time, state, parameters) {
        ...
        tau = ifelse(is.null(fixed_delay), parameters[["tau"]], fixed_delay)
        ...
        dPl = phi_Pl_past * L * (1 - alpha) - lambda * Pl - gamma * Pl
